...
def S_matrix(i: Int, A: Matrix, B: Matrix) -> Matrix:

    if i==1 or i==4 or i==7 or i==9:
        result = A-B
    elif i==2 or i==3 or i==5 or i==6 or i==8 or i==10:
        result = A+B
    else:
        raise ValueError('S matrix does not exist for index {}'.format(i))
...
def strassen_matrix_mult(A: Matrix, B: Matrix) -> Matrix:
    ...
    result.assign_submatrix(0, 0, P5 + P4 - P2 + strassen_matrix_mult(S(A,1,2,2,2), S(B,2,1,2,2)))
    result.assign_submatrix(0, result.num_of_cols//2, P1 + P2)
    result.assign_submatrix(result.num_of_rows//2, 0, P3 + P4)
    result.assign_submatrix(result.num_of_rows//2, result.num_of_cols//2, P5 + P1 - P3 - strassen_matrix_mult(S(A,1,1,2,1), S(B,1,1,1,2)))
