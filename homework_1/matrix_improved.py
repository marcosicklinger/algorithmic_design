...
def strassen_matrix_mult(A: Matrix, B: Matrix) -> Matrix:
    ...
    # store only the P_i that are used more than once to obtain final quadrants
    P1 = strassen_matrix_mult(get_matrix_quadrant(A,1,1), S_matrix(B,1,2,2,2))
    P2 = strassen_matrix_mult(S_matrix(A,1,1,1,2), get_matrix_quadrant(B,2,2))
    P3 = strassen_matrix_mult(S_matrix(A,2,1,2,2), get_matrix_quadrant(B,1,1))
    P4 = strassen_matrix_mult(get_matrix_quadrant(A,2,2), S_matrix(B,2,1,1,1))
    P5 = strassen_matrix_mult(S_matrix(A,1,1,2,2), S_matrix(B,1,1,2,2))

    result = Matrix([[0 for x in range(B.num_of_cols)] for y in range(A.num_of_rows)], clone_matrix=False)

    result.assign_submatrix(0, 0, P5 + P4 - P2 + strassen_matrix_mult(S_matrix(A,1,2,2,2), S_matrix(B,2,1,2,2)))
    result.assign_submatrix(0, result.num_of_cols//2, P1 + P2)
    result.assign_submatrix(result.num_of_rows//2, 0, P3 + P4)
    result.assign_submatrix(result.num_of_rows//2, result.num_of_cols//2, P5 + P1 - P3 - strassen_matrix_mult(S_matrix(A,1,1,2,1), S_matrix(B,1,1,1,2)))
